    ...
    () =>
      Array.isArray(users)
        ? users.map((auc) => {
            ...
            return {
              picture: auc?.profilePicture,
              fullName: auc?.fullName,
              userType: auc?.userType,
              city: auc?.city,
              phone: auc?.phone,
              email: auc?.email,
              actions: auc?._id,
    ...
    },
    {
      ...
      Cell: ({ row: { original } }) => (
        <div className="d-flex gap-2">
          <Link className="btn btn-outline-primary" to={`/seller/users/profile/${original.actions}`}>
            <i className="fa fa-eye"></i>
          </Link>
          <Link className="btn btn-outline-primary" to={`/seller/users/edit/${original.actions}`}>
      ...
      },
      {
        ...
        Cell: ({ row: { original } }) => (
          <div className="flex gap-2">
            <Link to={`/seller/users/profile/${original.actions}`}>
              <FaEye
                size={38}
            ...
            <Link to={`/seller/users/edit/${original.actions}`}>
              <FaRegEdit
                size={38}
                className="inline mt-[-3px] text-theme-color hover:text-white hover:bg-theme-color rounded-lg border-2 border-theme-color  px-2 py-[5px] transition-all"
              />
