...
import { getBidsAuctionsByUser, getBidsAuctionsWinnersParticipatedByUser } from "../../store/bid/bidSlice";
import { get_currency_symbol, convert_currency } from "../../currency";
...
const AuctionWinners = () => {
  ...
  const handleDeleteUser = (id) => {
    // dispatch(deleteSingleAuctionById(id)).then(() => {
  ...
  useEffect(() => {
    dispatch(getBidsAuctionsWinnersParticipatedByUser());
  }, [dispatch]);
...
const columns = useMemo(
  () => [
    ...
    },
    {
      ...
      accessor: "bidAmount",
      Cell: ({ value }) => <div className="">{convert_currency(value).value}</div>,      
    },
    {
      Header: "Bid Time",
      accessor: "bidTime",
      ...
      Cell: ({ row: { original } }) => (
        <div className="d-flex gap-2">
          ...
          {original.status === "upcoming" && (
            <Link className="btn btn-outline-primary" to={`/seller/auctions/edit/${original.actions}`}>
