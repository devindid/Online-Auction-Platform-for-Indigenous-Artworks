...
const BidsItem = () => {
  ...
  return (
    <div className="overflow-auto px-4 py-4 w-100 bg-dark text-white rounded-lg">
      <h2 className="text-white fw-bold fs-4 border-bottom border-light pb-3 mb-5">
        Bids Items
      </h2>
      <div className="overflow-auto px-3 bg-secondary rounded-lg max-height-750 border border-light">
        <table className="table table-striped table-bordered w-100">
          <thead className="sticky-top bg-primary text-white">
            <tr className="text-uppercase">
              <th className="rounded-start">Artwork</th>
              <th>Category</th>
              <th>Status</th>
              <th>Start Bid</th>
              <th>Your Bid</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr className="bg-dark text-center">
                <td colSpan="6" className="m-2 p-5 h-400">No Bids Made Yet.</td>
              </tr>
            ) : currentItems?.map((bid) => (
              <tr className="bg-dark" key={bid?._id}>
                <td className="align-middle rounded-start">
                  <div className="d-flex align-items-center gap-2">
                    ...
                    />
                    <span className="ps-2">{bid?.auction?.name}</span>
                  </div>
                </td>
                ...
                <td className="align-middle">
                  <span className="px-3 py-1 rounded-pill text-sm border bg-secondary border-light">
                    {bid?.auction?.status}
                  </span>
                </td>
                <td className="align-middle">
                ...
                <td className="align-middle rounded-end text-center">
                  <Link
                    className="text-primary hover-text-white hover-bg-primary rounded border border-primary p-2"
